...
# RAG Configuration
RAG_CHUNK_K = int(os.getenv('RAG_CHUNK_K', 8)) # Number of unique chunks to finally send to LLM
...
WEB_SEARCH_SUMMARIZATION_PROMPT = PromptTemplate(
    input_variables=["query", "web_results"],
    template="""
You are an expert academic assistant. Your task is to answer the user's query using ONLY the information from the provided web search results below. If a web result is not relevant to the query, ignore it completely.
...
**INSTRUCTIONS:**
1. **Relevance:** Carefully read each web result and use only those that are clearly relevant to the query. Ignore unrelated or off-topic results.
2. **Reasoning:** First, think step-by-step about how to answer the query using the relevant web results. Enclose your reasoning in <thinking> and </thinking> tags.
3. **Summarization:** After the </thinking> tag, write a clear, concise, and well-structured answer to the query, using only the relevant web results. Add more detail if the results provide it, but do not invent information.
4. **Citations:** When using information from a specific web result, cite it using [n], where n is the number of the web result (e.g., [1], [2]).
5. **No Fabrication:** If the web results do not contain enough information, state this clearly in your answer.
6. **Formatting:** Use Markdown for formatting (e.g., lists, bold, links) if appropriate.

**BEGIN RESPONSE (Start with the <thinking> tag):**
<thinking>
"""
)
