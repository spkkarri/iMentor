...
def extract_final_answer_and_reasoning(text):
    """
    Extracts the final answer and reasoning from the text.
    Expects format:
    Final Answer: <answer>
    Reasoning: <reasoning>

    Returns a tuple of (answer, reasoning).
    """
    if not isinstance(text, str):
        return "", ""
    match = re.search(r"Final Answer\s*:\s*(.*?)\s*Reasoning\s*:\s*(.*)", text, re.DOTALL | re.IGNORECASE)
    if match:
