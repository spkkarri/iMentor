...
function LogisticRegressionView() {
  ...
  const { data, w1, w2, b } = state.lr;

  useEffect(() => {
    dispatch({ type: 'SET_LR_DATA', payload: lrData });
  }, [dispatch]);
      ...
      <Grid container spacing={4}>
        ...
        <Grid item xs={12} md={6}>
          ...
          <ParameterSlider
            ...
            step={0.1}
          />
          <ControlButtons modelType="lr" />
        </Grid>
        <Grid item xs={12} md={6}>
          <ParameterDisplay modelType="lr" />
        </Grid>
      </Grid>
    </Box>
  );
...
export default LogisticRegressionView;