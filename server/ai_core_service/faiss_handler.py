...
def get_embedding_model():
    ...
    if embedding_model is None:
        if config.EMBEDDING_TYPE == 'sentence-transformer':
            ...
            # ==================================================================
            #  START OF THE DEFINITIVE FIX: Add local caching
            # ==================================================================
            
            # Define a cache directory within your project to store the model
            # This prevents re-downloading on every startup.
            ...
            try:
                ...
                if device == 'cpu':
                    logger.warning("CUDA not available. Using CPU for embeddings. This will be slow.")

                embedding_model = HuggingFaceEmbeddings(
                    model_name=config.EMBEDDING_MODEL_NAME,
                    model_kwargs={'device': device},
                    encode_kwargs={'normalize_embeddings': True},
                    cache_folder=cache_dir  # <-- THE CRITICAL FIX
                )
    ...
    try:
        ...
        for doc, score in all_results_with_scores:
            if not doc or not hasattr(doc, 'metadata') or not hasattr(doc, 'page_content'):
                ...
                continue
            if active_file:
                doc_name = doc.metadata.get("documentName", "").strip()
                if doc_name != active_file.strip():
                    continue  # ✅ Skip if this doc doesn't match the selected file
