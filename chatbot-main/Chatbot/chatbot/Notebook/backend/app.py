...
brcrypt = Bcrypt(app)  # Initialize Flask-Bcrypt for password hashing
...
def register():
    if request.method == 'POST':
        ...
        password = brcrypt.generate_password_hash(request.form['password']).decode('utf-8')

        if users.find_one({'username': username}):
            flash('Username already exists!')
            return redirect(url_for('register'))

        # Insert the new user with email
        users.insert_one({
            'username': username,
            'email': email,
            'password': password
...
@app.route('/index', methods=['GET', 'POST'])
def login():
    error = None  # For showing errors inside the template
    ...
    if request.method == 'POST':
        ...
        if user and brcrypt.check_password_hash(user['password'], password):
            session['username'] = user['username']
            return redirect(url_for('dashboard'))  # NEW: redirect to dashboard
        else:
