...
const ChatPage = ({ setIsAuthenticated }) => {
    ...
    const handleSendMessage = useCallback(async (e, overrideInput = null) => {
        ...
        try {
            const payload = { query: trimmedInput, history: [...messages, newUserMessage], sessionId, systemPrompt: editableSystemPromptText };
    ...
    }, [inputText, isProcessing, loadingStates, messages, sessionId, editableSystemPromptText, handleLogout]);
    ...
    const handleSaveEdit = useCallback(async () => {
        ...
        try {
            const payload = { query: editingMessage.text, history: [...historyBeforeEdit, updatedUserMessage], sessionId, systemPrompt: editableSystemPromptText };
            const response = await apiSendMessage(payload);
            const assistantMessage = {
                id: uuidv4(),
                role: 'assistant',
    ...
    }, [editingMessage, messages, sessionId, editableSystemPromptText]);
    ...
    return (
        <div className="chat-page-container">
            <div className="sidebar-area">
                <SystemPromptWidget selectedPromptId={currentSystemPromptId} promptText={editableSystemPromptText} onSelectChange={(id) => { setCurrentSystemPromptId(id); setEditableSystemPromptText(getPromptTextById(id)); }} onTextChange={(text) => { setEditableSystemPromptText(text); setCurrentSystemPromptId('custom'); }} />
                <FileUploadWidget onUploadSuccess={handleUploadSuccess} />
                <FileManagerWidget files={files} isLoading={loadingStates.files} error={fileError} onDeleteFile={handleDeleteFile} onRenameFile={handleRenameFile} onGenerateMindMap={handleGenerateMindMap} isProcessing={isProcessing} />
            </div>
            <div className="chat-container">
                ...
                <div className="modern-input-bar">
                    <div className="input-bar-left">
                        ...
                        <button type="button" className={`input-action-btn${isRagEnabled ? ' active' : ''}`} title="Chat with Files (RAG)" onClick={() => setIsRagEnabled(v => !v)} disabled={isProcessing}>RAG</button>
                    </div>
